...
def created_user_response(client: TestClient, user: User):
    ...
    client.delete("/auth/", params={"id": response.json()["id"]})


@pytest.fixture(scope="session")
def login_user2(client: TestClient, user2):
    response = client.post("/auth/register", json=user2.dict())
    ...
    yield LoginUser(
        access_token=data_login["access_token"], type_token=data_login["token_type"]
    )

    client.delete("/auth/", params={"id": response_data["id"]})
