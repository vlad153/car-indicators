...
class TestCreateUser:
    ...
    @given(
        ...
        password=text(min_size=4, alphabet=characters(blacklist_categories=['Cs'], blacklist_characters=['\x00'])), 
        )
...
async def test_request_verify(created_user_response: UserResponse, client, anyio_backend, 
                                                                monkeypatch):

    async def mock_on_after_request_verify(*args, **kwargs):
        return None
    
    monkeypatch.setattr(UserManager, "on_after_request_verify", 
...
@pytest.mark.parametrize('anyio_backend', ['asyncio'])
async def test_verification(created_user_response: UserResponse, client, anyio_backend, 
                                                                monkeypatch):
    token = 'some_token'
...
@pytest.mark.parametrize('anyio_backend', ['asyncio'])
async def test_request_reset_password(created_user_response: UserResponse, client: TestClient, anyio_backend, monkeypatch):
    
    async def mock_on_after_forgot_password(*args, **kwargs):
        return None
    
    monkeypatch.setattr(UserManager, "on_after_forgot_password", mock_on_after_forgot_password)
...
@pytest.mark.parametrize('anyio_backend', ['asyncio'])
async def test_reset_password(created_user_response: UserResponse, client: TestClient, anyio_backend, monkeypatch):
    ...
    response = client.post(
        '/auth/reset-password', 
        json={'token':'some_token', 'password':'new_password'}
        )
...
class TestLogout:
    ...
    def test_logout(login_user2: LoginUser, client: TestClient):
        ...
        response = client.post(
            '/auth/jwt/logout',
            headers={
                'Authorization': f'Bearer {login_user2.access_token}'
            }
            )
