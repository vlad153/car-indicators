...
class UserManager(UUIDIDMixin, BaseUserManager[User, uuid.UUID]):
    ...
    async def on_after_request_verify(
        ...
        logger.debug(
            "Verification email sent to user ({}) with token ({}).", user.email, token
        )
...
async def retrieve_user_manager() -> AsyncGenerator[UserManager, None]:
...
fastapi_users = FastAPIUsers[User, uuid.UUID](retrieve_user_manager, [auth_backend])  # type: ignore
